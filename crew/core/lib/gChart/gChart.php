...
/**
 * @mainpage
 * gChartPhp, a php wrapper for  the Google Chart Tools / Image Charts (aka Chart API) {@link https://code.google.com/apis/charttools/}
 *
 * @version 0.5.3
 */
...
class gLineChart extends gChart{
	...
	public function getUrl() {
		...
		return $retStr;
         ...
         //simpilot added features

        public function setlineMarkers($size = '8', $markerType = 'o', $markerColor = '000000', $seriesIndex = '0', $whichPoints = '-1') {
		$this -> setProperty('chm', $this -> encodeData(array($markerType,$markerColor,$seriesIndex,$whichPoints,$size), ','));
        }

        public function setlineWidth($lineWidth, $dashLength = '10', $spaceLength = '6') {
		$this -> setProperty('chls', $this -> encodeData(array($lineWidth,$dashLength,$spaceLength), ','));
	}

        public function setaxisStyle($axisIndex, $fontColor = '000000', $fontSize = '6') {
		$this -> setProperty('chxs', $this -> encodeData(array($axisIndex,$fontColor,$fontSize), ','));
	}

        //end simpilot added functions
}
...
class gMapChart extends gChart {
	 ...
	 *                          ISO 3166-1-alpha-2 codes for countries, {@link https://www.iso.org/iso/english_country_names_and_code_elements}
     *                          USA state code
	 */
